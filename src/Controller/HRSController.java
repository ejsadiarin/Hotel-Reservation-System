...
public class HRSController {
   ...
   */
  public void createHotel(JTextField hotelName, JTextField numOfRooms) {
    String hotelNameString = hotelName.getText();
    int numOfRoomsInt = Integer.parseInt(numOfRooms.getText());

    for (Hotel hotel : hotels) {
      if (hotel.getName().equals(hotelNameString)) {
        MessageHelper.showErrorMessage(String.format("Hotel name '%s' already exists.", hotelNameString));
        return;
      }
    }
    if (numOfRoomsInt < 1 || numOfRoomsInt > 50) 
      MessageHelper.showErrorMessage(String.format("'%d' number of rooms is not allowed (must be from 1 to 50 only).", numOfRoomsInt));
    else {
      hotels.add(new Hotel(hotelNameString, numOfRoomsInt));
      MessageHelper.showSuccessMessage(String.format("Hotel '%s' successfully created with %d rooms.", hotelNameString, numOfRoomsInt));
    }
